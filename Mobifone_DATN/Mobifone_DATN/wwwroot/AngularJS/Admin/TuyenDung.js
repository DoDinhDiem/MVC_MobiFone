myApp.controller('TuyenDungCtrl', ['$scope', '$http', '$window', function ($scope, $http, $window) {
    var token = localStorage.getItem('token');
    if (token) {

        // Khai báo base URL cho API
        var baseUrl = "/Admin/TuyenDung/";
        ...
        $scope.searchItem = function (Title, page, pageSize) {
            var baseUrl = "";
            var url = baseUrl + "Search_TuyenDung?title=" + Title + "&page=" + page + "&pageSize=" + pageSize;
            $http({
                method: 'get',
        ...
        $scope.CreateUpdate = function () {
            ...
            if ($scope.Id) {
                var Data = {
                    ...
                    LinkGoogleForm: $scope.LinkGoogleForm,
                    Image: $scope.Image
                };
                var url = baseUrl + "Update_TuyenDung";
                $http({
                    method: 'put',
                    headers: { "Authorization": 'Bearer ' + token },
                    url: url,
                ...
                var url = baseUrl + "Creat_TuyenDung";
                $http({
                    method: 'post',
                    headers: { "Authorization": 'Bearer ' + token },
                    url: url,
        ...
        $scope.getById = function (id) {
            var url = baseUrl + "GetById_TuyenDung?id=" + id;
            $http({
                method: 'get',
                headers: { "Authorization": 'Bearer ' + token },
                url: url
        ...
        $scope.deleteById = function (id) {
            if (confirm("Bạn có chắc chắn muốn xóa giới thiệu này không?")) {
                var url = baseUrl + "Delete_TuyenDung?id=" + id;
                $http({
                    method: 'delete',
                    headers: { "Authorization": 'Bearer ' + token },
                    url: url
